...
export function checkTurn(target, attacker, disfi, exit, itemlist, happy) {
  ...
  if (!checkDeath(target, attacker, disfi, exit, itemlist, happy)) {
    return;
  }

  if (checkStunTurn(target)) {
    checkDeath(target, attacker, disfi, exit, itemlist, happy);
    return;
  }
...
function checkEnemyDeath(target, attacker, func, res, itemlist, h) {
  if (attacker.chance != null) {
...
function checkDeath(target, attacker, func, res, itemlist, h) {
  if (target.version != null) {
    if (attacker.clife <= 0) {
      checkEnemyDeath(target, attacker, func, res, itemlist, h);
      return false;
    }
    if (target.clife <= 0) {
    ...
    if (target.clife <= 0) {
      checkEnemyDeath(attacker, target, func, res, itemlist, h);
      return false;
    }
  }
  return true;
